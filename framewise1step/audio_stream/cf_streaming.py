...
# This copy lives at <repo>/audio_stream, so the Causal-Forcing repo root is one level up.
CF_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
FRAME_SEQ = 1560
# Swapped to the Causal-Forcing++ framewise 1-step checkpoint (1 frame/block, 1 denoising step).
CKPT = "checkpoints/framewise/framewise-1step.pt"
CFG = "configs/causal_forcing_dmd_framewise_1step.yaml"
...
def load_cf_pipeline(window=21, sink=3, device=None):
    ...
    if isinstance(_ckpt, dict):
        for _k in ("generator_ema", "generator", "model", "state_dict"):
            if _k in _ckpt and isinstance(_ckpt[_k], dict):
                _ckpt = _ckpt[_k]
                break
    # framewise-1step.pt is an FSDP-wrapped EMA dict: keys look like
    # 'model._fsdp_wrapped_module.patch_embedding.weight'. Strip the wrapper prefixes so
    # they line up with WanDiffusionWrapper's 'model.patch_embedding.weight'.
    _clean = {}
    for _name, _w in _ckpt.items():
        _name = _name.replace("._fsdp_wrapped_module", "").replace("_fsdp_wrapped_module.", "")
        if _name.startswith("module."):
            _name = _name[len("module."):]
        _clean[_name] = _w
    _missing, _unexpected = gen.load_state_dict(_clean, strict=False)
    print(f"[cf_streaming] framewise-1step loaded: {len(_clean)} params | "
          f"missing={len(_missing)} unexpected={len(_unexpected)}")
    if _unexpected:
        print("  unexpected[:5]:", list(_unexpected)[:5])
    if _missing:
        print("  missing[:5]:", list(_missing)[:5])
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default="/mnt/data/SPED/gino/out/cf_live/smoke.mp4")
    args = ap.parse_args()
