...
"""
import os, sys, time, re, base64, io, queue
import numpy as np
import torch
...
MAX_SECONDS = 90  # uncapped (note: CF RoPE drifts past ~30s — quality degrades late in a long run)
RAMP_CHUNKS = 4   # forward conditioning-ramp horizon (chunks) on a prompt change (~3s ramp)
import subprocess
...
def gen_loop(start_prompt):
    ...
    for c in range(n_chunks):
        ...
        p, _ = bus.get()
        if p and p != cur:
            gen.hardcut(p); cur = p   # HARD CUT (no recache, no ramp)
