...
def main():
    ...
    prompt = args.prompt
    # --- mid-rollout prompt swap (env-driven, training-free) ---
    # SWAP_PROMPT : the new prompt to switch to mid-rollout
    # SWAP_AT     : action index (0-based) at which to start using SWAP_PROMPT
    # SWAP_RESET_REF=1 : after the swap, stop re-anchoring ref_latents to the
    #                    pre-swap scene (freeze the style anchor at swap moment)
    swap_prompt = os.environ.get("SWAP_PROMPT", "").strip()
    swap_at = int(os.environ.get("SWAP_AT", "0") or "0")
    swap_reset_ref = os.environ.get("SWAP_RESET_REF", "0") == "1"
    if swap_prompt:
        logger.info(f"[SWAP] will switch to '{swap_prompt}' at action idx {swap_at} (reset_ref={swap_reset_ref})")
    ...
    for idx, action_id in enumerate(action_list):
        ...
        # Pick the prompt for this chunk: swap to the new prompt once we reach swap_at.
        cur_prompt = swap_prompt if (swap_prompt and idx >= swap_at) else prompt
        # Apply history-attenuation levers POST-SWAP only (read by the pipeline per step).
        if swap_prompt and idx >= swap_at:
            os.environ["GC_HIST_SCALE"] = os.environ.get("SWAP_HIST", "1.0")
            os.environ["GC_GT_SCALE"] = os.environ.get("SWAP_GT", "1.0")
            os.environ["GC_MASK_SCALE"] = os.environ.get("SWAP_MASK", "1.0")
        logger.info(f"Generating segment {idx+1}/{len(action_list)} with action ID: {action_id} | prompt='{cur_prompt[:60]}...'")
        ...
        # SWAP_RESET_REF: once swapped, freeze the style-anchor ref_latents so it
        # stops pulling the rollout back toward the pre-swap scene. last_latents
        # (continuity) is always updated so motion stays coherent.
        if not (swap_reset_ref and swap_prompt and idx >= swap_at):
