...
# local_attn_size=21 (the training horizon) turns on the rolling sliding-window
# KV cache so rollouts can exceed the 21-latent-frame (~5s) global-cache limit;
# for <=21 frames it is identical to the original global attention. sink_size=1
# pins the first frame as an anchor to slow long-horizon drift.
transformer = WanDiffusionWrapper(is_causal=True, local_attn_size=21, sink_size=1)
...
def generate_video_stream(prompt, seed, enable_torch_compile=False, enable_fp8=False, use_taehv=False, num_blocks=7):
    ...
    try:
        ...
        # Generation parameters (num_blocks comes from the request -> duration)
        num_blocks = max(1, int(num_blocks))
        noise = torch.randn([1, num_blocks * pipeline.num_frame_per_block, 16, 60, 104],
                            device=gpu, dtype=torch.float16, generator=rnd)
...
def handle_start_generation(data):
    ...
    # Duration (seconds of generated video) -> number of 3-frame latent blocks.
    # Native video is 16fps and the VAE upsamples time 4x, so latent_frames ~=
    # seconds*4 and blocks = round(latent_frames / 3). Capped to keep VRAM/time sane.
    duration_seconds = float(data.get('duration', 5))
    num_blocks = max(1, min(80, round(duration_seconds * 16 / 4 / 3)))
    ...
    socketio.start_background_task(generate_video_stream, prompt, seed,
                                   enable_torch_compile, enable_fp8, use_taehv, num_blocks)
