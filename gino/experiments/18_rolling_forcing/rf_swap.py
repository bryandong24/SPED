...
@torch.no_grad()
def rolling_swap(pipe, noise, cond_a, cond_b, swap_block, kv_reset=False, recache=0, ramp=0, full_reset=False):
    ...
    for wi in range(window_num):
        ...
        if (not swapped) and eb >= swap_block:
            ...
            for c in pipe.crossattn_cache:
                c["is_init"] = False
            if recache > 0:
                # rebuild clean cache from the last `recache` committed frames under new prompt
                pipe._initialize_kv_cache(b, noise.dtype, dev)
                s0 = max(0, cs - recache)
                fi = s0
                while fi < cs:
                ...
                pipe._initialize_kv_cache(b, noise.dtype, dev)  # wipe sink + history
                for c in pipe.crossattn_cache:
                    c["is_init"] = False
            if full_reset:
                pipe._initialize_kv_cache(b, noise.dtype, dev)
                noisy_cache[:, cs:] = 0  # also drop the in-flight (sun-noised) staircase
                for c in pipe.crossattn_cache:
                    c["is_init"] = False
            swapped = True
...
def main():
    ...
    frames = rolling_swap(pipe, noise, cond_a, cond_b, swap_block,
                          kv_reset=args.kv_reset, recache=args.recache, ramp=args.ramp, full_reset=args.full_reset)
