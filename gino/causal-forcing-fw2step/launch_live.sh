...
set +e
cd /data/SPED/gino/causal-forcing-fw2step || exit 2
pkill -9 -f "causal-forcing-fw2step/web_live_cf.py" 2>/dev/null
sleep 2
...
nohup env CUDA_VISIBLE_DEVICES="$GPUS" "$PY" web_live_cf.py --port "$PORT" --asr_gpu 1 \
  > /data/SPED/cf_fw2step_live.log 2>&1 &
echo "live server launched PID $! (GPUs=$GPUS [video=cuda:0, asr=cuda:1], port $PORT, log=/data/SPED/cf_fw2step_live.log)"
