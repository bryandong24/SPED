...
pkill -9 -f "causal-forcing-fw2step/demo.py" 2>/dev/null
sleep 2
...
nohup env CUDA_VISIBLE_DEVICES="$GPUS" "$PY" demo.py --port "$PORT" \
  > /data/SPED/cf_fw2step_ui.log 2>&1 &
echo "UI server launched PID $! (GPU=$GPUS, port $PORT, log=/data/SPED/cf_fw2step_ui.log)"
