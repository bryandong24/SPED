...
# ==============================================================================
import inspect
import os  # for swap-resistance history-attenuation knobs (GC_HIST_SCALE/GC_GT_SCALE/GC_MASK_SCALE)
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # --- history-attenuation knobs (default 1.0 = stock behaviour) ---
                # Used to weaken the old-scene anchor so a mid-rollout prompt swap can take.
                #   GC_HIST_SCALE : scale the clean history clamped into the first-half latents
                #   GC_GT_SCALE   : scale the gt_latents reference channel
                #   GC_MASK_SCALE : scale the 1=history/0=predict mask channel
                _hist = float(os.environ.get("GC_HIST_SCALE", "1.0"))
                _gt   = float(os.environ.get("GC_GT_SCALE", "1.0"))
                _mask = float(os.environ.get("GC_MASK_SCALE", "1.0"))
                if last_latents.shape[2] == 1:
                    latents[:,:,0,:,:] = last_latents[:,:,-1,:,:]
                else:
                    latents[:,:,:latents.shape[2]//2,:,:] = last_latents * _hist
                    gt_latents_concat[:,:,:latents.shape[2]//2,:,:] = last_latents

                # expand the latents if we are doing classifier free guidance
                latents_concat = torch.concat([latents, gt_latents_concat * _gt, mask_concat * _mask], dim=1)
